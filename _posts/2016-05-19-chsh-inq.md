...
Here
\begin{equation}
S=E(\textbf{AB})-E(\textbf{A,B'})+E(\textbf{A'B'})+E(\textbf{A'B}) \ ,
\end{equation}
where \\(E(\textbf{AB})\\) - is a correlation function (for example, spin-spin correlation function) between two points on different sides of the source and  arbitary unit vectors (\\(\textbf{A}\\), \\(\textbf{B}\\)) specify the orientations of the linear detectors.
