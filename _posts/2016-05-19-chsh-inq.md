...
### Detectors
---
 Since the configuration of detectors can arbitrary, we choose to constrain angles between them as \\(\angle(\textbf{A, B})=\angle(\textbf{B, A'})=\angle(\textbf{A',B' })=\theta\\). Then \\(S\\) becomes only a function of \\(\theta \\)
